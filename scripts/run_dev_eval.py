"""Run the six development prompt conditions on the balanced dev subset.
...
DEV_FILE = PROJECT_DIR / "prepared_data" / "dev_subset.jsonl"
PROMPT_FILE = PROJECT_DIR / "prompts" / "prompt_pack.json"
OUT_FILE = PROJECT_DIR / "results" / "dev" / "full_dev_results.jsonl"
...
def build_relation_block(mode: str, pack: dict) -> str:
    if mode == "label_only":
        return "Allowed relation labels:\n" + "\n".join(f"- {x}" for x in pack["labels"])
    if mode == "short_definition":
        defs = pack["short_definitions"]
    else:
        defs = pack["rich_definitions"]
...
def build_prompt(example: dict, prompt_condition: str, pack: dict) -> str:
    definition_mode, evidence_mode = prompt_condition.split("__")
...
{build_relation_block(definition_mode, pack)}
...
def main() -> None:
    ...
    for ex in dev_examples:
        for cond in pack["prompt_conditions"]:
